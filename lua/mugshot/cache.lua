...
local function key_for(avatar)
  if avatar.login then return "gh-" .. avatar.login end
...
function M.fetch(avatar, cb)
  if not avatar or not avatar.url then return cb(nil) end
  ...
  vim.system({ "curl", "-fsSL", avatar.url, "-o", path }, { text = true }, function(dl)
    ...
    vim.system(
      ...
      { text = true },
      function(mg)
        vim.schedule(function() cb(mg.code == 0 and path or nil) end)
      end
    )
  end)
end
