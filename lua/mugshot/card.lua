...
local function rel_time(epoch)
  ...
  if d < 60 then return ago(d, "second") end
  if d < 3600 then return ago(d / 60, "minute") end
  if d < 86400 then return ago(d / 3600, "hour") end
  if d < 86400 * 7 then return ago(d / 86400, "day") end
  if d < 86400 * 30 then return ago(d / (86400 * 7), "week") end
  if d < 86400 * 365 then return ago(d / (86400 * 30), "month") end
...
local function build_lines(info, cap, opts)
  ...
  if opts.hint_row then
    ...
    lines[#lines + 1] = ""
    lines[#lines + 1] =
      ("%s open · %s copy · %s pr · %s close"):format(a.open_commit, a.copy_sha, a.open_pr, dismiss)
  end
  return lines
end
...
function M.open(info, cwd)
  ...
  local function close()
    if img then pcall(function() img:clear() end) end
  ...
  local function map(lhs, fn)
    for _, k in ipairs(type(lhs) == "table" and lhs or { lhs }) do
      if k then vim.keymap.set("n", k, fn, { buffer = buf, nowait = true, silent = true }) end
    end
  end
  map(a.dismiss, close)
  map(a.open_commit, function() github.open_commit(info, cwd) end)
  map(a.copy_sha, function() github.copy_sha(info.sha) end)
  map(a.open_pr, function() github.open_pr(info, cwd) end)
    ...
    resolve.resolve(
      ...
      function(av)
        cache.fetch(av, function(path)
          if not path or not vim.api.nvim_win_is_valid(win) then return end
          ...
          vim.api.nvim_create_autocmd({ "WinScrolled", "VimResized" }, {
            buffer = buf,
            callback = function() pcall(function() img:render() end) end,
          })
        end)
      end
    )
