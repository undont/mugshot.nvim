...
local function kitty_capable()
  if vim.env.MUGSHOT_FORCE_KITTY == "1" then return true end
  if vim.env.KITTY_WINDOW_ID then return true end
  if vim.env.GHOSTTY_RESOURCES_DIR or vim.env.GHOSTTY_BIN_DIR then return true end
  if vim.env.WEZTERM_PANE then return true end
...
local function tmux_passthrough()
  ...
  if val == "on" or val == "all" then return true end
  return false, ("tmux allow-passthrough is '%s'; set it to 'on' for avatars"):format(val == "" and "off" or val)
end

---@param force? boolean
---@return mugshot.Capability
function M.detect(force)
  if cached and not force then return cached end

  local image = has_image()
  local terminal = kitty_capable()
